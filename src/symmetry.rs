...
pub fn starting_orbits<M: Mask>(grid: &GridDefinition, blocks: &[M]) -> Vec<(usize, u128)> {
    ...
    let active: Vec<usize> = bounds
        .iter()
        .enumerate()
        .filter_map(|(axis, &(lo, hi))| (lo < hi).then_some(axis))
        .collect();
    for &axis in &active {
        ...
        if let Some(permutation) = permutation {
            join_verified(&mut parent, blocks, &permutation);
        }
    }
    if base > 0 {
        let permutation = map_points(grid, &index, |point| {
...
fn join_verified<M: Mask>(parent: &mut [usize], blocks: &[M], permutation: &[usize]) {
    let n = parent.len();
    for a in 0..n {
        for b in 0..n {
