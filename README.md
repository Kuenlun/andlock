...
# Counts and totals beyond u128.
andlock --free-points 35 --big-counts --json
```
...
`andlock 3x3 --min-length 4 --json` emits one count report containing `grid`, `requested_range`, `completed_range`, `counts`, `total`, and `status`. Counts and totals are decimal strings, preserving full precision. Each count has `length` and `count` fields. Ranges have inclusive `min_length` and `max_length` fields.
...
Counts use `u128` by default, with checked overflow and finalized results retained before a failure. `--big-counts` enables arbitrary precision for counts and totals while keeping compact fixed-width DP entries. It may choose deeper prefix partitions so each continuation fits `u128`, increasing runtime on constrained grids. Each count needs at most 710 bits and the total at most 711 bits for the supported 127 points, separately from the counting-table budget.
